...
#+END_SRC
   ...
   Theoretically =wavetable-quality-factor= of 2 should be enough to rebuild
   sinusoid without artifacts (hello Nyquist-Shannon), but in my experiments 16
   was the lowest value for clear signal. Is it because of table being
   non-time-aligned? If so, interpolating table should help.
...
#+BEGIN_SRC scheme
  ...
  (define (make-control x)
    (let-values ([(signal setter) (ctrl:make-control x)])
      (values
