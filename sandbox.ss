    ...
    (let ([frequency (~< frequency)])
      (let-values ([(start end) (fix-duration (* 0.25 duration))])
        (let ([s (*~
          ...
          (my-lpf (echo (*~ (~< 0.3) (amp2phase (osc:sine* 0.05))) (~< 0.9) s))
          ;; (my-lpf (echo (~< 0.3) (~< 0.8) s))
          ;; (my-lpf (echo (~< 0.05) (~< 0.5) s))
          ;; s
          ))))

  (define-values (my-dsp1 play-note1) (make-polyphony 6 make-note1))

  (define my-dsp my-dsp1))

(begin
  (define my-pat1 '(1 0 1 0 1))
  (define my-pat2 '(0 1)))
...
(define (play-my-perc)
  (let ([beat (time->beat (now) *bpm*)])
    (play-pattern my-pat1 (cut play-note1 440.0 (/ 32.0)) beat)
    (play-pattern my-pat2 (cut play-note1 880.0 (/ 32.0)) beat)
